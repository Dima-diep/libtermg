...
std::vector<unsigned int> checkmenu(const std::string title, const std::vector<std::string> select, const unsigned int selectp=6) {
    static unsigned int rselect = 0;
    static std::vector<unsigned int> retsel = {};
    static bool ok = true;
	...
	WINDOW * win = newwin(rselect + 4, maxlen + 6, maxy / 2 - (rselect + 4) / 2, maxx / 2 - maxlen / 2 - 3);
	keypad(win, true);
	wrefresh(win);
	refresh();
	box(win, 0, 0);
	wrefresh(win);
	refresh();
	WINDOW * wtitle = newwin(3, maxlen + 6, maxy / 2 - (rselect + 4) / 2, maxx / 2 - maxlen / 2 - 3);
	refresh();
	box(wtitle, 0, 0);
	wrefresh(wtitle);
	refresh();
	...
	wrefresh(wtitle);
	refresh();
	WINDOW * okc = newwin(3, maxlen + 6, maxy / 2 + (rselect + 2) / 2, maxx / 2 - maxlen / 2 - 3);
	refresh();
	box(okc, 0, 0);
	wrefresh(okc);
	refresh();
		...
		if (ch == KEY_UP) {
			...
			if (cursel > 1) {
			    ...
			    for (unsigned int k=0; k < rselect; k++) {
			    	...
			    	it = std::find(retsel.begin(), retsel.end(), allsel + k);
			    	if (it != retsel.end()) {
			    		wmove(win, 3 + k, 3);
			    		waddch(win, '+');
			    	}
			...
			if (cursel < rselect) {
			    ...
			    for (unsigned int k=0; k < rselect; k++) {
			    	...
			    	waddstr(win, printed[k].c_str());
			    	std::vector<unsigned int>::iterator it;
			    	it = std::find(retsel.begin(), retsel.end(), allsel - rselect + 1 + k);
			    	if (it != retsel.end()) {
			    		wmove(win, 3 + k, 3);
			    		waddch(win, '+');
			    	}
			...
			wmove(win, 2 + cursel, 3);
			std::vector<unsigned int>::iterator it;
			it = std::find(retsel.begin(), retsel.end(), allsel);
			if (it != retsel.end()) {
		    	waddch(win, ' ');
		    	retsel.erase(std::find(retsel.begin(), retsel.end(), allsel));
		    } else {
		    	waddch(win, '+');
		    	retsel.push_back(allsel);
		    }
		    wrefresh(win);
		    refresh();
		}
